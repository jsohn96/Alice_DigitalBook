...
public class Book : MonoBehaviour {
	...
	[SerializeField] Image[] _arrowImages;
	[SerializeField] FocusBook _focusBookScript;

    [SerializeField]
    RectTransform BookPanel;
    public Sprite background;
	public Sprite _endBackground;
    ...
    void Update()
    {
		...
		if (currentPage == 0) {
			_arrowImages [0].enabled = false;
			_arrowImages [1].enabled = true;
		} else if (currentPage == TotalPageCount) {
			_arrowImages [0].enabled = true;
			_arrowImages [1].enabled = false;
		} else {
			_arrowImages [0].enabled = true;
			_arrowImages [1].enabled = true;
		}
    ...
    public void DragRightPageToPoint(Vector3 point)
    {
		...
		Right.sprite = (currentPage < bookPages.Length - 1) ? bookPages[currentPage + 1] : _endBackground;

		RightNext.sprite = (currentPage < bookPages.Length - 2) ? bookPages[currentPage + 2] : _endBackground;
    ...
    {
		if (interactable) {
			_focusBookScript._disableFocus = true;
	...
	public void DragLeftPageToPointFirst(Vector3 point)
	{
		if (currentPage <= 0) return;
		pageDragging = true;
		mode = FlipMode.LeftToRight;
    ...
    void UpdateSprites()
    {
		...
		RightNext.sprite=(currentPage>=0 &&currentPage<bookPages.Length) ? bookPages[currentPage] : _endBackground;
    }
    ...
    void Flip()
    {
		...
		if (currentPage == 2) {
			Events.G.Raise (new ToturialStateEvent (1));
		} else if (currentPage == 8) {
			Events.G.Raise (new ToturialStateEvent (3));
		}
    }
    ...
    public IEnumerator TweenTo(Vector3 to, float duration, System.Action onFinish)
    {
		...
		if (onFinish != null) {
			_focusBookScript._disableFocus = false;
			interactable = true;
			onFinish ();
		}
    }
